...
st.write("In this section we illustrate the clustering using the K-means method")
...
st.image(Image.open('./Media/Silhouette Analysis.png'), use_container_width=True)
...
st.image(Image.open("./Media/Bounderies of clusters.png"), use_container_width=True)

st.write("And mapped out the silhouette score, to take a look at whether the clusters were good or not.")

st.image(Image.open("./Media/Silhouette Plot of KMeans.png"), use_container_width=True)

st.success("A silhouette score around 0.5 and higher is acceptable result. We imagine that the score would be higher if there wasn't so many entries close to the borders between the clusters. Choosing different method of clustering may produce better results. ")